...
function initializeDatabase() {
    db.serialize(() => {
        ...
        // Tabla de entradas (compras/agregado de stock)
        db.run(`
                ...
                db.get('SELECT COUNT(*) as count FROM usuarios', async (err, row) => {
                    if (!err && row.count === 0) {
                        try {
                            ...
                            const cajeroPass = await bcrypt.hash('123456', 10);
                            db.run(`
                                INSERT INTO usuarios (username, password, nombre, rol)
                                VALUES 
                                ('admin', ?, 'Administrador Tienda CVS', 'ADMIN'),
                                ('cajero', ?, 'Cajero Tienda CVS', 'CAJERO')
                            `, [adminPass, cajeroPass]);
                            console.log('Usuarios iniciales creados (admin/admin123, cajero/123456)');
                        } catch (e) {
                            console.error('Error al encriptar contraseñas iniciales:', e);
                        }
                    }
                });
            }
        ...
        // Sembrar productos de prueba para Tienda CVS si la tabla está vacía
